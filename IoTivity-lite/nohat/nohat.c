...
/*
 ...
 input_file            : /home/pi/workspace/nohat/device_output/out_codegeneration_merged.swagger.json
 version of input_file : 20190222
 title of input_file   : NoHat
*/
...
app_init(void)
{
  ...
  ret |= oc_add_device("/oic/d", "oic.d.nohat", "NoHat", 
                       "ocf.2.0.5", /* icv value */
...
post_light1(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  ...
  if (error_state == false)
  {
    ...
    digitalWrite(LED_ACT, g_light1_value);	// set GPIO pin

    //also set ACT (green) LED on Pi board
    char msgOut[MAX_PAYLOAD_STRING];
    snprintf(msgOut, sizeof(msgOut), "echo %d | sudo tee /sys/class/leds/led0/brightness", (int)g_light1_value);
    system(msgOut);
...
int init;
  ...
  PRINT("Used input file : \"/home/pi/workspace/nohat/device_output/out_codegeneration_merged.swagger.json\"\n");
  PRINT("OCF Server name : \"NoHat\"\n");

  /*intialize the variables */
  initialize_variables();

  // enable GPIO access to LED on Pi board
  ...
  pinMode (LED_ACT, OUTPUT) ;         // BCM_GPIO pin set as output
  ...
  */
  ...
  PRINT("OCF server \"NoHat\" running, waiting on incoming connections.\n");

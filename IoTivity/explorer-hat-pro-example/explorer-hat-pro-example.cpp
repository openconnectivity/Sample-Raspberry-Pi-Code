...
/*
 ...
 input_file            : /home/pi/workspace/explorerhatexample/device_output/out_codegeneration_merged.swagger.json
...
/*
 * class definition for class that handles /light
 *
...
class LightResource : public Resource
{
    public:
         ...
         */
        LightResource(std::string resourceUri = "/light");

        /*
         * destructor
         */
         virtual ~LightResource(void);
    ...
    private:
        ...
        /*
         * Make the payload for the retrieve function (e.g. GET) /light
         * This resource describes a binary switch (on/off).
         * The value is a boolean.
         * A value of 'true' means that the switch is on.
         * A value of 'false' means that the switch is off.
        ...
        /*
         * Parse the payload for the update function (e.g. POST) /light
        ...
        // member variables for path: "/light"
...
* Constructor code
*/
LightResource::LightResource(std::string resourceUri)
{
    std::cout << "- Running: LightResource constructor" << std::endl;

    m_resourceUri = resourceUri;
    // initialize member variables /light
...
OCStackResult LightResource::registerResource(uint8_t resourceProperty)
{
    ...
    {
        std::cerr << "Failed to register LightResource." << std::endl;
        return result;
    }
    ...
    std::cout << "LightResource:" << std::endl;
...
/*
* Make the payload for the observe function (e.g. GET) /light
*/
OCStackResult LightResource::sendNotification(void)
{
...
*/
OCStackResult LightResource::sendNotification(const std::shared_ptr< OCResourceResponse > pResponse)
{
    OCStackResult sResult = OC_STACK_OK;
    if ( m_interestedObservers.size() > 0) {
        std::cout << "Notifying list "  << m_interestedObservers.size() << " of observers\n";
...
* @param queries  the query parameters for this call
*/
OCRepresentation LightResource::get(QueryParamsMap queries)
{
    OC_UNUSED(queries);
...
* @return OCEntityHandlerResult ok or not ok indication
*/
OCEntityHandlerResult LightResource::post(QueryParamsMap queries, const OCRepresentation& rep)
{
...
*/
bool LightResource::in_updatable_interfaces(std::string interface_name)
{
...
OCEntityHandlerResult LightResource::entityHandler(std::shared_ptr<OCResourceRequest> request)
{
    ...
    //std::cout << "In entity handler for LightResource " << std::endl;

    if(request)
    {
        std::cout << "In entity handler for LightResource, URI is : "
        ...
        if(requestFlag & RequestHandlerFlag::RequestFlag)
        {
            ...
            {
                std::cout<<"LightResource Get Request"<< std::endl;
            ...
            {
                std::cout <<"LightResource Post Request"<<std::endl;
            ...
            {
                std::cout << "LightResource unsupported request type (delete,put,..)"
...
/*
 * class definition for class that handles /switch
 *
 * This resource describes a binary switch (on/off).
 * The value is a boolean.
 * A value of 'true' means that the switch is on.
...
class SwitchResource : public Resource
{
    public:
         ...
         * @param resourceUri the uri for this resource
         */
        SwitchResource(std::string resourceUri = "/switch");

        /*
         * destructor
         */
         virtual ~SwitchResource(void);
    ...
    private:
        ...
        /*
         * Make the payload for the retrieve function (e.g. GET) /switch
         * This resource describes a binary switch (on/off).
         * The value is a boolean.
         * A value of 'true' means that the switch is on.
         * A value of 'false' means that the switch is off.
        ...
        // member variables for path: "/switch"
        std::vector<std::string>  m_var_value_if; // the value for the array attribute "if": The interface set supported by this resource
        std::string m_var_name_if = "if"; // the name for the attribute "if"
        std::string m_var_value_n; // the value for the attribute "n": Friendly name of the resource
        std::string m_var_name_n = "n"; // the name for the attribute "n"
...
* Constructor code
*/
SwitchResource::SwitchResource(std::string resourceUri)
{
    std::cout << "- Running: SwitchResource constructor" << std::endl;

    m_resourceUri = resourceUri;
    // initialize member variables /switch
    // initialize vector if  The interface set supported by this resource
    m_var_value_if.push_back("oic.if.baseline");
    m_var_value_if.push_back("oic.if.a");
    m_var_value_n = "";  // current value of property "n" Friendly name of the resource
...
OCStackResult SwitchResource::registerResource(uint8_t resourceProperty)
{
    OCStackResult result = OC_STACK_ERROR;
    EntityHandler cb = std::bind(&SwitchResource::entityHandler, this,PH::_1);
    result = OCPlatform::registerResource(m_resourceHandle,
                                          m_resourceUri,
                                          m_RESOURCE_TYPE[0],
                                          m_RESOURCE_INTERFACE[0],
    ...
    if(OC_STACK_OK != result)
    {
        std::cerr << "Failed to register SwitchResource." << std::endl;
        return result;
    }
    ...
    std::cout << "SwitchResource:" << std::endl;
    std::cout << "\t" << "# resource interfaces: "
              << sizeof(m_RESOURCE_INTERFACE)/sizeof(m_RESOURCE_INTERFACE[0]) << std::endl;
    std::cout << "\t" << "# resource types     : "
              << sizeof(m_RESOURCE_TYPE)/sizeof(m_RESOURCE_TYPE[0]) << std::endl;
...
/*
* Make the payload for the observe function (e.g. GET) /switch
*/
OCStackResult SwitchResource::sendNotification(void)
{
    OCStackResult sResult = OC_STACK_OK;
    if ( m_interestedObservers.size() > 0) {
        std::cout << "Notifying list "  << m_interestedObservers.size() << " of observers\n";
...
* @param pResponse  the response to use for the observe
*/
OCStackResult SwitchResource::sendNotification(const std::shared_ptr< OCResourceResponse > pResponse)
{
    OCStackResult sResult = OC_STACK_OK;
    if ( m_interestedObservers.size() > 0) {
        std::cout << "Notifying list "  << m_interestedObservers.size() << " of observers\n";
...
* @param queries  the query parameters for this call
*/
OCRepresentation SwitchResource::get(QueryParamsMap queries)
{
    OC_UNUSED(queries);
...
* Check if the interface name is an registered interface name
*/
bool SwitchResource::in_updatable_interfaces(std::string interface_name)
{
    for (unsigned int i=0; i < (sizeof(m_IF_UPDATE)/sizeof(m_IF_UPDATE[0])); i++)
    {
        if (m_IF_UPDATE[i].compare(interface_name) == 0)
...
* the entity handler
*/
OCEntityHandlerResult SwitchResource::entityHandler(std::shared_ptr<OCResourceRequest> request)
{
    OCEntityHandlerResult ehResult = OC_EH_ERROR;
    //std::cout << "In entity handler for SwitchResource " << std::endl;

    if(request)
    {
        std::cout << "In entity handler for SwitchResource, URI is : "
                  << request->getResourceUri() << std::endl;
        ...
        if(requestFlag & RequestHandlerFlag::RequestFlag)
        {
            ...
            if(request->getRequestType() == "GET")
            {
                std::cout<<"SwitchResource Get Request"<< std::endl;
            ...
            {
                std::cout << "SwitchResource unsupported request type (delete,put,..)"
                    << request->getRequestType() << std::endl;
                pResponse->setResponseResult(OC_EH_ERROR);
                OCPlatform::sendResponse(pResponse);
                ehResult = OC_EH_ERROR;
...
class IoTServer
{
    ...
    private:
        LightResource  m_lightInstance;
        SwitchResource  m_switchInstance;};
...
{
    OCStackResult result = OC_STACK_ERROR;
    result = m_lightInstance.registerResource(resourceProperty);
    if(OC_STACK_OK != result)
    {
        return result;
    }
    result = m_switchInstance.registerResource(resourceProperty);
    if(OC_STACK_OK != result)
    {
        return result;
    }return result;

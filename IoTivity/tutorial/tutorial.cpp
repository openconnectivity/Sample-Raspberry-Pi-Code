...
/*
 ...
 input_file            : /home/cstevens1/workspace/tutorial/device_output/out_codegeneration_merged.swagger.json
...
/*
 * class definition for class that handles /dimmingswitch
 *
...
*/
class DimmingswitchResource : public Resource
{
    public:
        /*
         * constructor
         ...
         * @param resourceUri the uri for this resource
         */
        DimmingswitchResource(std::string resourceUri = "/dimmingswitch");

        /*
         * destructor
         */
         virtual ~DimmingswitchResource(void);
...
//        void dimmingswitchObserverLoop();
    ...
    private:
        ...
        /*
         * Parse the payload for the update function (e.g. POST) /dimmingswitch
        ...
        // member variables for path: "/dimmingswitch"
        int m_var_value_dimmingSetting; // the value for the attribute "dimmingSetting": Current dimming value
        std::string m_var_name_dimmingSetting = "dimmingSetting"; // the name for the attribute "dimmingSetting"
        std::vector<std::string>  m_var_value_if; // the value for the array attribute "if": The interface set supported by this resource
        std::string m_var_name_if = "if"; // the name for the attribute "if"
        std::string m_var_value_n; // the value for the attribute "n": Friendly name of the resource
        std::string m_var_name_n = "n"; // the name for the attribute "n"
...
* Constructor code
*/
DimmingswitchResource::DimmingswitchResource(std::string resourceUri)
{
    std::cout << "- Running: DimmingswitchResource constructor" << std::endl;

    m_resourceUri = resourceUri;
    // initialize member variables /dimmingswitch
    m_var_value_dimmingSetting = 30; // current value of property "dimmingSetting" Current dimming value
    // initialize vector if  The interface set supported by this resource
    m_var_value_if.push_back("oic.if.a");
    m_var_value_if.push_back("oic.if.baseline");
    m_var_value_n = "";  // current value of property "n" Friendly name of the resource
...
* Destructor code
*/
DimmingswitchResource::~DimmingswitchResource() { }

OCStackResult DimmingswitchResource::registerResource(uint8_t resourceProperty)
{
    OCStackResult result = OC_STACK_ERROR;
    EntityHandler cb = std::bind(&DimmingswitchResource::entityHandler, this,PH::_1);
    result = OCPlatform::registerResource(m_resourceHandle,
                                          m_resourceUri,
                                          m_RESOURCE_TYPE[0],
                                          m_RESOURCE_INTERFACE[0],
    ...
    if(OC_STACK_OK != result)
    {
        std::cerr << "Failed to register DimmingswitchResource." << std::endl;
        return result;
    }
    ...
    std::cout << "DimmingswitchResource:" << std::endl;
    std::cout << "\t" << "# resource interfaces: "
              << sizeof(m_RESOURCE_INTERFACE)/sizeof(m_RESOURCE_INTERFACE[0]) << std::endl;
    std::cout << "\t" << "# resource types     : "
              << sizeof(m_RESOURCE_TYPE)/sizeof(m_RESOURCE_TYPE[0]) << std::endl;
...
/*
* Make the payload for the observe function (e.g. GET) /dimmingswitch
*/
OCStackResult DimmingswitchResource::sendNotification(void)
{
    OCStackResult sResult = OC_STACK_OK;
    if ( m_interestedObservers.size() > 0) {
        std::cout << "Notifying list "  << m_interestedObservers.size() << " of observers\n";
...
* @param pResponse  the response to use for the observe
*/
OCStackResult DimmingswitchResource::sendNotification(const std::shared_ptr< OCResourceResponse > pResponse)
{
    OCStackResult sResult = OC_STACK_OK;
    if ( m_interestedObservers.size() > 0) {
        std::cout << "Notifying list "  << m_interestedObservers.size() << " of observers\n";
...
* @param queries  the query parameters for this call
*/
OCRepresentation DimmingswitchResource::get(QueryParamsMap queries)
{
    OC_UNUSED(queries);
...
* @return OCEntityHandlerResult ok or not ok indication
*/
OCEntityHandlerResult DimmingswitchResource::post(QueryParamsMap queries, const OCRepresentation& rep)
{
    OCEntityHandlerResult ehResult = OC_EH_OK;
    OC_UNUSED(queries);
    try {
    ...
    }if (ehResult == OC_EH_OK)
    {
        ...
        try {
            // value exist in payload
            int temp;
            if (rep.getValue(m_var_name_dimmingSetting, temp ))
            {
                m_var_value_dimmingSetting = temp;
                std::cout << "\t\t" << "property 'dimmingSetting' UPDATED: " << m_var_value_dimmingSetting << std::endl;
            }
            else
            {
...
* Check if the interface name is an registered interface name
*/
bool DimmingswitchResource::in_updatable_interfaces(std::string interface_name)
{
    for (unsigned int i=0; i < (sizeof(m_IF_UPDATE)/sizeof(m_IF_UPDATE[0])); i++)
    {
        if (m_IF_UPDATE[i].compare(interface_name) == 0)
...
* the entity handler
*/
OCEntityHandlerResult DimmingswitchResource::entityHandler(std::shared_ptr<OCResourceRequest> request)
{
    OCEntityHandlerResult ehResult = OC_EH_ERROR;
    //std::cout << "In entity handler for DimmingswitchResource " << std::endl;

    if(request)
    {
        std::cout << "In entity handler for DimmingswitchResource, URI is : "
                  << request->getResourceUri() << std::endl;
        ...
        if(requestFlag & RequestHandlerFlag::RequestFlag)
        {
            ...
            if(request->getRequestType() == "GET")
            {
                std::cout<<"DimmingswitchResource Get Request"<< std::endl;
            ...
            {
                std::cout <<"DimmingswitchResource Post Request"<<std::endl;
                bool  handle_post = true;
            ...
            {
                std::cout << "DimmingswitchResource unsupported request type (delete,put,..)"
                    << request->getRequestType() << std::endl;
                pResponse->setResponseResult(OC_EH_ERROR);
                OCPlatform::sendResponse(pResponse);
                ehResult = OC_EH_ERROR;
...
/*
 * class definition for class that handles /lightstate
 *
 * This resource describes a dimming function.
 * The value is an integer showing the current dimming level.
 * If step (from oic.r.baseresource) is present then it represents the increment between dimmer values.
 ...
 * Retrieves the current dimming level.
*/
class LightstateResource : public Resource
{
    public:
        /*
         * constructor
         ...
         * @param resourceUri the uri for this resource
         */
        LightstateResource(std::string resourceUri = "/lightstate");

        /*
         * destructor
         */
         virtual ~LightstateResource(void);
    ...
    private:
        ...
        /*
         * Make the payload for the retrieve function (e.g. GET) /lightstate
         * This resource describes a dimming function.
         * The value is an integer showing the current dimming level.
         * If step (from oic.r.baseresource) is present then it represents the increment between dimmer values.
         * When range (from oic.r.baseresource) is omitted, then the range is [0,100].
        ...
        /*
         * Parse the payload for the update function (e.g. POST) /lightstate
         * Sets the desired dimming level.
         * @param queries  the query parameters for this call
         * @param rep  the response to get the property values from
         * @return OCEntityHandlerResult ok or not ok indication
        ...
        // member variables for path: "/lightstate"
        int m_var_value_dimmingSetting; // the value for the attribute "dimmingSetting": Current dimming value
        std::string m_var_name_dimmingSetting = "dimmingSetting"; // the name for the attribute "dimmingSetting"
        std::vector<std::string>  m_var_value_if; // the value for the array attribute "if": The interface set supported by this resource
        std::string m_var_name_if = "if"; // the name for the attribute "if"
...
* Constructor code
*/
LightstateResource::LightstateResource(std::string resourceUri)
{
    std::cout << "- Running: LightstateResource constructor" << std::endl;

    m_resourceUri = resourceUri;
    // initialize member variables /lightstate
    m_var_value_dimmingSetting = 30; // current value of property "dimmingSetting" Current dimming value
    // initialize vector if  The interface set supported by this resource
    m_var_value_if.push_back("oic.if.a");
    m_var_value_if.push_back("oic.if.baseline");
...
* Destructor code
*/
LightstateResource::~LightstateResource() { }

OCStackResult LightstateResource::registerResource(uint8_t resourceProperty)
{
    OCStackResult result = OC_STACK_ERROR;
    EntityHandler cb = std::bind(&LightstateResource::entityHandler, this,PH::_1);
    result = OCPlatform::registerResource(m_resourceHandle,
                                          m_resourceUri,
                                          m_RESOURCE_TYPE[0],
                                          m_RESOURCE_INTERFACE[0],
    ...
    if(OC_STACK_OK != result)
    {
        std::cerr << "Failed to register LightstateResource." << std::endl;
        return result;
    }
    ...
    std::cout << "LightstateResource:" << std::endl;
    std::cout << "\t" << "# resource interfaces: "
              << sizeof(m_RESOURCE_INTERFACE)/sizeof(m_RESOURCE_INTERFACE[0]) << std::endl;
    std::cout << "\t" << "# resource types     : "
              << sizeof(m_RESOURCE_TYPE)/sizeof(m_RESOURCE_TYPE[0]) << std::endl;
...
/*
* Make the payload for the observe function (e.g. GET) /lightstate
*/
OCStackResult LightstateResource::sendNotification(void)
{
    OCStackResult sResult = OC_STACK_OK;
    if ( m_interestedObservers.size() > 0) {
        std::cout << "Notifying list "  << m_interestedObservers.size() << " of observers\n";
...
* @param pResponse  the response to use for the observe
*/
OCStackResult LightstateResource::sendNotification(const std::shared_ptr< OCResourceResponse > pResponse)
{
    OCStackResult sResult = OC_STACK_OK;
    if ( m_interestedObservers.size() > 0) {
        std::cout << "Notifying list "  << m_interestedObservers.size() << " of observers\n";
...
* @param queries  the query parameters for this call
*/
OCRepresentation LightstateResource::get(QueryParamsMap queries)
{
    OC_UNUSED(queries);
...
* @return OCEntityHandlerResult ok or not ok indication
*/
OCEntityHandlerResult LightstateResource::post(QueryParamsMap queries, const OCRepresentation& rep)
{
    OCEntityHandlerResult ehResult = OC_EH_OK;
    OC_UNUSED(queries);
    try {
...
* Check if the interface name is an registered interface name
*/
bool LightstateResource::in_updatable_interfaces(std::string interface_name)
{
    for (unsigned int i=0; i < (sizeof(m_IF_UPDATE)/sizeof(m_IF_UPDATE[0])); i++)
    {
        if (m_IF_UPDATE[i].compare(interface_name) == 0)
...
* the entity handler
*/
OCEntityHandlerResult LightstateResource::entityHandler(std::shared_ptr<OCResourceRequest> request)
{
    OCEntityHandlerResult ehResult = OC_EH_ERROR;
    //std::cout << "In entity handler for LightstateResource " << std::endl;

    if(request)
    {
        std::cout << "In entity handler for LightstateResource, URI is : "
                  << request->getResourceUri() << std::endl;
        ...
        if(requestFlag & RequestHandlerFlag::RequestFlag)
        {
            ...
            if(request->getRequestType() == "GET")
            {
                std::cout<<"LightstateResource Get Request"<< std::endl;
            ...
            {
                std::cout <<"LightstateResource Post Request"<<std::endl;
                bool  handle_post = true;
            ...
            {
                std::cout << "LightstateResource unsupported request type (delete,put,..)"
                    << request->getRequestType() << std::endl;
                pResponse->setResponseResult(OC_EH_ERROR);
                OCPlatform::sendResponse(pResponse);
                ehResult = OC_EH_ERROR;
...
class IoTServer
{
    ...
    private:
        ...
        DimmingswitchResource  m_dimmingswitchInstance;
        LightstateResource  m_lightstateInstance;};

IoTServer::IoTServer()
    :m_binaryswitchInstance(),
     m_dimmingswitchInstance(),
     m_lightstateInstance()
{
...
OCStackResult IoTServer::registerResources(uint8_t resourceProperty)
{
    ...
    result = m_dimmingswitchInstance.registerResource(resourceProperty);
    if(OC_STACK_OK != result)
    {
        return result;
    }
    result = m_lightstateInstance.registerResource(resourceProperty);
    if(OC_STACK_OK != result)
    {
        return result;
    }return result;
...
public:
    ...
    std::string  deviceName = "Dimming";
    std::string  deviceType = "oic.d.switchdevice";
